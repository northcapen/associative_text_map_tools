...
def build_noteto(title, content=None):
    return NoteTO(note=Note(title=title, content=content, guid=str(uuid.uuid4())), notebook=None, status="success")
...
def test_fix_links():
    ...
    content = en_note
    p = LinkFixer(
        note_guid_to_titles_dict={"eac75a87-f509-4eb3-a53c-9718cc6437d9": build_noteto(title="B_newname").note},
        notes_trash=None,
    )
...
def test_evernote_with_embeded():
    ...
    p = LinkFixer(note_guid_to_titles_dict={"b": build_noteto(title="B_newname").note}, notes_trash={})
